...
import { requireAdmin, requireAdminOrFaculty, setupAuth } from "./auth";
import { db } from "./db";
...
Central Luzon State University
  ...
  app.post("/api/reset-password", async (req, res) => {
    try {
      ...
      // Hash new password using scrypt (consistent with login)
    ...
    async (req: any, res) => {
      try {
        ...
        if (currentUser.role === "admin") {
          ...
          const profSessions = todaySessions.filter(
  ...
  app.post("/api/classrooms", requireAdmin, async (req, res) => {
    try {
      ...
      if (classroomData.location)
        classroomData.location = classroomData.location.trim();

      // Check for duplicate classroom name
      const allClassrooms = await storage.getClassrooms();
      const duplicateName = allClassrooms.find(
        (c) =>
          c.name.toLowerCase() === classroomData.name.toLowerCase() &&
          c.isActive
      );
      if (duplicateName) {
        return res
  ...
  app.put("/api/classrooms/:id", requireAdmin, async (req, res) => {
    try {
      ...
      if (
        updateData.name &&
        updateData.name.toLowerCase() !== existingClassroom.name.toLowerCase()
      ) {
        const allClassrooms = await storage.getClassrooms();
        const duplicateName = allClassrooms.find(
          (c) =>
  ...
  app.put("/api/subjects/:id", requireAdminOrFaculty, async (req: any, res) => {
    try {
      const id = parseInt(req.params.id);
      const professorId = req.user.id;
      const subjectData = insertSubjectSchema
        .partial()
        .parse({ ...req.body, professorId });
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch students in subject" });
      }
    }
  );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch student enrollments" });
      }
    }
  );
  ...
  app.post("/api/enrollments", requireAdminOrFaculty, async (req: any, res) => {
    try {
      ...
      const subject = allSubjects.find(
        (s) => s.id === enrollmentData.subjectId
      );
      if (!subject) {
      ...
      const duplicate = existingEnrollments.find(
        (e) => e.subjectId === enrollmentData.subjectId && e.isActive
      );
      if (duplicate) {
        return res
          .status(400)
  ...
  app.post("/api/schedules", requireAdminOrFaculty, async (req: any, res) => {
    try {
      ...
      if (scheduleData.subjectName && !scheduleData.subjectId) {
        ...
        const existingSubject = await storage.getSubjectByName(
      ...
      if (scheduleData.autoPopulate !== false) {
        const { populateScheduleSessions } = await import(
          "./services/scheduleService"
        );
        const populateResult = await populateScheduleSessions(schedule.id);
        console.log(
          `📅 Auto-populated ${populateResult.sessionsCreated} sessions for schedule ${schedule.id}`
        );
      }
  ...
  app.put(
    ...
    async (req: any, res) => {
      try {
        const id = parseInt(req.params.id);
        const professorId = req.user.id;
        const scheduleData = insertScheduleSchema
          .partial()
          .parse({ ...req.body, professorId });
  ...
  app.get("/api/computers", requireAdminOrFaculty, async (req: any, res) => {
    try {
      ...
      let computers;
      if (classroomId) {
        computers = await storage.getComputersByClassroom(
          parseInt(classroomId as string)
        );
      } else {
  ...
  app.post("/api/computers", requireAdminOrFaculty, async (req: any, res) => {
    try {
      const professorId = req.user.id;
      const computerData = insertComputerSchema.parse({
        ...req.body,
        professorId,
      });
      ...
      if (computerData.classroomId) {
        const existingComputers = await storage.getComputersByClassroom(
          computerData.classroomId
        );
        const duplicate = existingComputers.find(
          (c) =>
  ...
  app.put(
    "/api/computers/:id/assign",
    requireAdminOrFaculty,
    async (req, res) => {
      try {
        const computerId = parseInt(req.params.id);
        const { studentId } = req.body;
        const computer = await storage.assignComputerToStudent(
          computerId,
          studentId
        );
        res.json(computer);
    ...
    async (req: any, res) => {
      try {
        ...
        if (!BREVO_API_KEY) {
          return res
            .status(400)
            .json({ message: "Brevo API key not configured" });
        }

        const FROM_EMAIL = process.env.FROM_EMAIL || "matt.feria@clsu2.edu.ph";

        const { TransactionalEmailsApi, TransactionalEmailsApiApiKeys } =
          await import("@getbrevo/brevo");
        ...
        const message = {
          ...
          textContent:
            "This is a test email from CLIRDEC Presence system using Brevo.",
          htmlContent:
            "<p>This is a test email from <strong>CLIRDEC Presence system</strong> using Brevo.</p>",
        };
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        const { studentId, recipientType, subject, message, priority, type } =
          req.body;
        const currentUser = req.user;
        ...
        if (!recipientEmail) {
          return res
            .status(400)
            .json({ message: "No email found for recipient" });
        }
        ...
        <div style="padding: 20px;">
          ...
          <p style="color: #666; font-size: 12px;">
            This message was sent regarding ${student.firstName} ${
          student.lastName
        } (${student.studentId})
        ...
        if (BREVO_API_KEY) {
          try {
              ...
              to: [
                {
                  ...
                  name:
                    recipientType === "parent" ? "Parent/Guardian" : "Student",
                },
              ],
              sender: {
                email: FROM_EMAIL,
            ...
            if (
              ...
              errorMessage =
                "Brevo authentication failed. Please verify your API key is valid and active.";
            } else if (error.response?.status === 400) {
              errorMessage =
                "Brevo request error. Please verify the 'from' email address is verified in your Brevo account.";
            } else if (error.response?.status === 429) {
              errorMessage =
                "Brevo daily sending limit exceeded. Please wait or upgrade your plan.";
            }
            ...
            res.status(500).json({
              ...
              error: error.message,
              suggestion:
                "In Brevo dashboard: 1) Verify your sender email, 2) Check API key is active",
            });
          }
        } else {
          // No API key available
          console.log(
            `❌ No Brevo API key - email not sent to ${recipientEmail}`
          );
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        const summary = {
          present: allAttendanceRecords.filter((r) => r.status === "present")
            .length,
          late: allAttendanceRecords.filter((r) => r.status === "late").length,
          absent: allAttendanceRecords.filter((r) => r.status === "absent")
            .length,
            ...
            if (record.studentId) {
              ...
              if (student?.parentEmail) {
                await sendEmailNotification(
                  ...
                  "absence_alert",
                  `${student.firstName} ${student.lastName} was absent from ${
                    session.startTime || "class session"
    ...
    async (req: any, res) => {
      try {
        ...
        if (!studentId || !recipientType || !subject || !message) {
          return res.status(400).json({
            message:
              "Missing required fields: studentId, recipientType, subject, and message are required",
          });
        }
        ...
        console.log(
          `📧 Creating email notifications for ${recipients.length} recipients`
        );
        ...
        for (const recipient of recipients) {
          try {
            ...
            console.error(
              `❌ Failed to create notification for ${recipient.email}:`,
              error
            );
          }
        }
        ...
        if (notificationsCreated > 0) {
          ...
          setImmediate(async () => {
            try {
              console.log(
                `🔄 Processing email queue for ${notificationsCreated} notifications...`
              );
              const { processEmailQueue } = await import(
          ...
          res.status(500).json({
            message:
              "Failed to create any email notifications. Please check server logs for details.",
          });
        }
      } catch (error) {
  ...
  app.get(
    "/api/attendance/behavior-analysis",
    requireAdminOrFaculty,
    async (req: any, res) => {
      try {
        const { analyzeStudentAttendanceBehavior } = await import(
        ...
        for (const student of students) {
          const behavior = await analyzeStudentAttendanceBehavior(
            student.id
          ).catch((error) => {
            console.error(
              `Error analyzing behavior for student ${student.id}:`,
              error
            );
        ...
        behaviorAnalysis.sort((a, b) => {
          ...
          return (
            (priorities[a.behaviorLevel] ?? 5) -
            (priorities[b.behaviorLevel] ?? 5)
          );
        });
        ...
        res
          .status(500)
          .json({ message: "Failed to analyze attendance behavior" });
      }
    }
  );
  ...
  app.post(
    "/api/notifications/send-parent-email",
    requireAdminOrFaculty,
    async (req: any, res) => {
      try {
  ...
  app.post(
    "/api/attendance/mark-intervention",
    requireAdminOrFaculty,
    async (req: any, res) => {
      try {
        const { studentId } = req.body;
        ...
        console.log(
          `Intervention marked for student: ${studentId} by user: ${req.user?.email}`
        );
  ...
  app.post(
    "/api/attendance/trigger-monitoring",
    requireAdmin,
    async (req: any, res) => {
      try {
        const { checkAllStudentsAttendanceBehavior } = await import(
          "./services/attendanceMonitor"
        );
        ...
        res
          .status(500)
          .json({ message: "Failed to trigger attendance monitoring" });
      }
    }
  );
  ...
  app.get(
    "/api/settings/attendance-monitoring",
    requireAdminOrFaculty,
    async (req: any, res) => {
      try {
        const settings = {
          enabled: (await storage.getSystemSetting(
            "attendance_monitoring_enabled"
          )) || { value: "true" },
          thresholds: {
            critical: (await storage.getSystemSetting(
              "attendance_threshold_critical"
            )) || { value: "50" },
            concerning: (await storage.getSystemSetting(
              "attendance_threshold_concerning"
            )) || { value: "60" },
            consecutiveAbsences: (await storage.getSystemSetting(
              "consecutive_absences_threshold"
            )) || { value: "3" },
            lateArrivalsWeekly: (await storage.getSystemSetting(
              "late_arrivals_weekly_threshold"
            )) || { value: "3" },
          },
          notifications: {
            cooldownDays: (await storage.getSystemSetting(
              "alert_cooldown_days"
            )) || { value: "7" },
            checkInterval: (await storage.getSystemSetting(
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch monitoring settings" });
      }
    }
  );
      ...
      try {
        ...
        if (enabled !== undefined) {
          await storage.setSystemSetting(
            "attendance_monitoring_enabled",
            enabled.toString(),
            "Enable/disable automated attendance monitoring"
          );
        }
        ...
        if (thresholds) {
          if (thresholds.critical) {
            await storage.setSystemSetting(
              ...
              "Attendance rate threshold for critical alerts"
            );
          }
          if (thresholds.concerning) {
            await storage.setSystemSetting(
              "attendance_threshold_concerning",
              thresholds.concerning.toString(),
              "Attendance rate threshold for concerning alerts"
            );
          }
          if (thresholds.consecutiveAbsences) {
            await storage.setSystemSetting(
              "consecutive_absences_threshold",
              thresholds.consecutiveAbsences.toString(),
              "Number of consecutive absences that trigger alerts"
            );
          }
          if (thresholds.lateArrivalsWeekly) {
            await storage.setSystemSetting(
              "late_arrivals_weekly_threshold",
              thresholds.lateArrivalsWeekly.toString(),
              "Number of late arrivals per week that trigger alerts"
            );
          }
        }
        ...
        if (notifications) {
          if (notifications.cooldownDays) {
            await storage.setSystemSetting(
              "alert_cooldown_days",
              notifications.cooldownDays.toString(),
          ...
          if (notifications.checkInterval) {
            await storage.setSystemSetting(
              "monitoring_check_interval",
              notifications.checkInterval.toString(),
              "Hours between automated monitoring checks"
            );
          }
        }
        ...
        res
          .status(500)
          .json({ message: "Failed to update monitoring settings" });
      }
    }
  );
  ...
  const wss = new WebSocketServer({
    ...
    verifyClient: (info: { origin: string; req: any; secure: boolean }) => {
      ...
      const allowedOrigins: string[] = [`http://${host}`, `https://${host}`];
      ...
      try {
        ...
        if (memoryReport.status === "CRITICAL") {
          recommendations.push(
            "IMMEDIATE: Memory usage is critical - emergency cleanup recommended"
          );
        } else if (memoryReport.status === "HIGH") {
          recommendations.push(
            "WARNING: High memory usage detected - monitoring closely"
          );
        } else {
          recommendations.push(
  ...
  app.post(
    "/api/system/force-memory-cleanup",
    requireAdmin,
    async (req, res) => {
      try {
        const { EmergencyMemoryOptimizer } = await import(
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch optimization settings" });
      }
    }
  );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to update device configuration" });
      }
    }
  );
    ...
    async (req, res) => {
      try {
        ...
        if (success) {
          res.json({
            success: true,
            message: "Diagnostics request sent to device",
          });
        } else {
          res.status(404).json({ message: "Device not found or offline" });
        }
  ...
  app.post("/api/iot/broadcast", requireAdmin, async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: "Message broadcasted to all devices",
      });
    } catch (error) {
  ...
  app.post(
    "/api/retention-policies/cleanup",
    requireAdmin,
    async (req: any, res) => {
      try {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const attendanceRecords = await storage.getAttendanceByStudent(
          studentId
        );
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename=student_${student.studentId}_data_export.json`
        );
        res.json(exportData);
      } catch (error) {
  ...
  app.get("/api/export/parent/:email", async (req, res) => {
    try {
      ...
      const parentStudents = allStudents.filter(
        (s) => s.parentEmail?.toLowerCase() === parentEmail && s.isActive
      );
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=parent_${parentEmail}_data_export.json`
      );
      res.json(exportData);
    } catch (error) {
