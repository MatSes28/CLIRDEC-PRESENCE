...
export default function EnhancedRFIDSimulator() {
  ...
  const [readerStatus, setReaderStatus] = useState<'online' | 'offline' | 'scanning'>('online');
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const sampleCards = ['RFID001', 'RFID002', 'RFID003', 'RFID004', 'RFID005'];
  ...
  const getStatusIcon = () => {
    switch (readerStatus) {
      ...
      case 'scanning': return <Radar className="h-4 w-4 animate-spin" />;
      ...
      <CardContent className="space-y-6">
        ...
        <div className="relative">
          <div className={`
            ...
            ${readerStatus === 'scanning' ? 'border-blue-500 bg-blue-50' : 'border-gray-300 bg-gray-50'}
            flex flex-col items-center justify-center transition-all duration-300
            ${tapAnimation ? 'scale-105 shadow-lg' : ''}
          `}>
            <div className={`
              w-16 h-16 rounded-full border-4 mb-2
              ${readerStatus === 'scanning' ? 'border-blue-500 animate-pulse' : 'border-gray-400'}
            ...
            <p className="text-sm text-gray-600 font-medium">
              {readerStatus === 'scanning' ? 'Scanning...' : 'Tap RFID Card Here'}
            </p>
          </div>
          ...
          `}>
            ...
            {lastTap.status && (
              <Badge variant="secondary" className="mt-2">
                {lastTap.status.charAt(0).toUpperCase() + lastTap.status.slice(1)}
              </Badge>
            )}
          </div>
        )}
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <Button 
                ...
                disabled={rfidTapMutation.isPending || readerStatus === 'scanning'}
                size="sm"
              >
          ...
          <div className="space-y-3 pt-4 border-t">
            <div className="flex items-center justify-between">
              <div>
                <Label className="text-sm font-medium">Proximity Validation</Label>
                <p className="text-xs text-muted-foreground">Require physical proximity sensor validation</p>
              </div>
              <Switch
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Simulate random student taps every 5 seconds</p>
              </div>
              <Switch
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Simulate reader online/offline</p>
              </div>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setReaderStatus(readerStatus === 'online' ? 'offline' : 'online')}
              >
                Toggle Status
              </Button>
            </div>
