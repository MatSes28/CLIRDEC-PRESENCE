...
export default function AuthPage() {
  ...
  const loginMutation = useMutation({
    mutationFn: async (credentials: LoginData) => {
      console.log("AuthPage: Attempting login with credentials:", credentials);
      const res = await apiRequest("POST", "/api/login", credentials);
      console.log("AuthPage: Login response status:", res.status);
      return await res.json();
    },
    onSuccess: (data) => {
      console.log("AuthPage: Login successful, user data:", data);
      queryClient.invalidateQueries({ queryKey: ["/api/user"] });
      setLocation("/");
    },
    onError: (error: Error) => {
      console.log("AuthPage: Login failed with error:", error);
