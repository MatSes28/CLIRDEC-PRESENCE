...
export default function DiscrepancyDashboard() {
  ...
  // Fetch discrepancies
  const { data: discrepanciesData, isLoading: discrepanciesLoading, refetch: refetchDiscrepancies } = useQuery({
  ...
  const getDiscrepancyIcon = (flag: string) => {
    switch (flag) {
      case 'ghost_tap': return <UserX className="h-4 w-4" />;
      case 'sensor_without_rfid': return <Zap className="h-4 w-4" />;
      case 'validation_timeout': return <Clock className="h-4 w-4" />;
  ...
  const getDiscrepancyColor = (flag: string) => {
    switch (flag) {
      case 'ghost_tap': return 'destructive';
      case 'sensor_without_rfid': return 'secondary';
      case 'validation_timeout': return 'outline';
      case 'system_error': return 'destructive';
  ...
  const getDiscrepancyDescription = (flag: string) => {
    switch (flag) {
      case 'ghost_tap': return 'RFID tap detected without physical presence validation';
      case 'sensor_without_rfid': return 'Sensor detected movement without RFID tap';
      case 'validation_timeout': return 'RFID tap occurred but 7-second validation window expired';
      case 'system_error': return 'Technical error during validation process';
      case 'checkout_without_checkin': return 'Student attempted checkout without prior check-in';
      case 'already_checked_in': return 'Multiple check-in attempts detected';
      default: return 'Unknown discrepancy detected';
    }
  };
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              {discrepancies.filter(d => d.discrepancyFlag === 'ghost_tap').length}
            </div>
            <p className="text-xs text-muted-foreground">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {discrepancies.length > 0 
                ? Math.round((1 - discrepancies.length / (discrepancies.length + 100)) * 100)
                : 98
              }%
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <TabsContent value="discrepancies" className="space-y-4">
          {discrepanciesLoading ? (
            ...
            <Alert>
              ...
              <AlertDescription>
                No attendance discrepancies found today. All validations are working correctly!
              </AlertDescription>
            </Alert>
          ) : (
            <div className="space-y-4">
              {discrepancies.map((record: DiscrepancyRecord) => (
                <Card key={record.id} className="border-l-4 border-l-destructive">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <CardDescription>
                            Student ID: {record.student.studentId} | Session: {record.session.id}
                          </CardDescription>
                        </div>
                      </div>
                      <Badge variant={getDiscrepancyColor(record.discrepancyFlag) as any}>
                        {record.discrepancyFlag.replace(/_/g, ' ').toUpperCase()}
                      </Badge>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-4">
                      ...
                      {record.sensorDetectionTime ? (
                        <div>
                          ...
                          <div className="text-green-600">
                            {format(new Date(record.sensorDetectionTime), 'HH:mm:ss')}
                          </div>
                        </div>
                      ) : (
                        <div>
                          ...
                          <div className="text-destructive">
                            {record.validationTimeout ? 'Timeout (7s exceeded)' : 'No detection'}
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="text-xs text-muted-foreground">
                        Detected {formatDistanceToNow(new Date(record.createdAt))} ago
                      </div>
        ...
        <TabsContent value="pending" className="space-y-4">
          {pendingLoading ? (
            ...
            <Alert>
              <CheckCircle className="h-4 w-4" />
              <AlertDescription>
                No pending validations. All RFID taps are being processed normally.
              </AlertDescription>
            </Alert>
          ) : (
            <div className="space-y-4">
              {pendingValidations.map((validation: PendingValidation, index: number) => (
                <Card key={validation.validationKey || index} className="border-l-4 border-l-orange-500">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <CardDescription>
                            Session: {validation.sessionId} | Status: {validation.status}
                          </CardDescription>
                        </div>
                      </div>
                  ...
                  <CardContent>
                    <div className="flex items-center justify-between">
                      <div className="text-sm">
                        <strong>RFID Tap:</strong> {format(new Date(validation.rfidTapTime), 'HH:mm:ss')}
                        <br />
                      ...
                      <div className="text-xs text-muted-foreground">
                        {formatDistanceToNow(new Date(validation.rfidTapTime))} ago
                      </div>
                    </div>
                  </CardContent>
                </Card>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Breakdown of validation issues</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {Object.entries(
                    discrepancies.reduce((acc: any, record: DiscrepancyRecord) => {
                      acc[record.discrepancyFlag] = (acc[record.discrepancyFlag] || 0) + 1;
                      return acc;
                    }, {})
                  ).map(([flag, count]) => (
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium text-red-600">
                      {((discrepancies.filter(d => d.discrepancyFlag === 'ghost_tap').length / Math.max(discrepancies.length, 1)) * 100).toFixed(1)}%
                    </span>
                  </div>
